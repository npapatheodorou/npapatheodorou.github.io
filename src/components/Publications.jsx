  ...
  return (
    <div className="flex items-center gap-1.5">
      <div className={boxSize + ' rounded flex items-center justify-center overflow-hidden border border-surface-700/20 ' + brand.bg}>
        <img src={brand.src} alt={brand.alt + ' logo'} className={iconSize + ' object-contain'} loading="lazy" />
      </div>
...
var PublicationCard = ({ pub, isExpanded, onToggle }) => {
  var typeStyles = {
    'Journal Article': { bg: 'bg-purple-500/10', text: 'text-purple-500' },
    'Conference Paper': { bg: 'bg-blue-500/10', text: 'text-blue-500' },
    'Book Chapter': { bg: 'bg-green-500/10', text: 'text-green-500' },
  };
...
var Publications = function() {
  ...
  var stats = {
    ...
    journals: PUBLICATIONS_DATA.filter(function(pub) { return pub.type === 'Journal Article'; }).length,
    conferences: PUBLICATIONS_DATA.filter(function(pub) { return pub.type === 'Conference Paper'; }).length,
    chapters: PUBLICATIONS_DATA.filter(function(pub) { return pub.type === 'Book Chapter'; }).length,
  };

  var publishers = [];
  var seen = {};
  PUBLICATIONS_DATA.forEach(function(pub) {
    if (!seen[pub.publisher]) {
      seen[pub.publisher] = true;
      publishers.push(pub.publisher);
    }
  });
